...
| 1일 배치 - 실행 | 기준시간을 근거로 1시간 단위 분석을 24회 이하로 진행 후 일간 분석 (`HourlyMonitorService` → `DailyMonitorService` 순서 실행, 선후관계 처리 중요) | `DailyMonitorService` 정상 종료 시 COMPLETED | 테스트 화면에서는 `testDailyMonitorJob` 실행 (운영 `dailyMonitorJob`과 분리) |
